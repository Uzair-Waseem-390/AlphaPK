...
# ---------------------------------------------------------------------------
# Advance capped at confirmation must also correct cash_in_hand
# ---------------------------------------------------------------------------

class AdvanceCapCashSyncTests(BillingTestBase):
    """
    An advance is collected against a DRAFT, before line items and totals are
    locked in, so confirm_invoice caps it at grand_total if the total came out
    lower. That cap used to update the Payment row and the drawer event but
    NOT CashFlow.cash_in_hand — the full uncapped advance had already been
    added at draft creation, so the capped-off difference stayed in the
    counter permanently, with no error and no way to notice.

    Unbounded, not a rounding artifact: a 50,000 advance on a draft later
    reduced to an 850 total left cash overstated by 49,150.

    The invariant these tests hold: CashFlow.cash_in_hand must always equal
    the net of the CashMovement event table (cash-in-hand.md — the drawer
    reads only the events, so a divergence means the total on screen no longer
    matches the movements listed under it).
    """

    def _cash_counter_vs_events(self):
    ...
    def _advance_invoice(self, product, *, quantity, advance):
        invoice = create_invoice(
            customer_id=self.customer.id,
            items=[{"product_id": product.id, "quantity": quantity}],
    ...
    def test_capped_advance_reduces_cash_in_hand(self):
        product = self.make_stocked_product(stock=20, unit_cost="50", selling_price="100")
        # 2 x 100 = 200 total, but 5,000 was taken as an advance.
        invoice = self._advance_invoice(product, quantity=2, advance="5000")

        self.assertEqual(invoice.grand_total, Decimal("200.0000"))
        self.assertEqual(invoice.advance_amount, Decimal("200.0000"),
                         "advance should be capped down to grand_total")

        counter, events = self._cash_counter_vs_events()
        self.assertEqual(
            counter, events,
            f"cash counter {counter} disagrees with the event table {events} — "
            f"the capped-off 4,800 was left in cash_in_hand",
        )

    def test_uncapped_advance_is_untouched(self):
        """Guards against over-correcting: when the advance does NOT exceed
        grand_total there is nothing to cap and nothing to reverse."""
        product = self.make_stocked_product(stock=20, unit_cost="50", selling_price="100")
        invoice = self._advance_invoice(product, quantity=5, advance="300")

        self.assertEqual(invoice.grand_total, Decimal("500.0000"))
        self.assertEqual(invoice.advance_amount, Decimal("300.0000"),
                         "a partial advance must stay exactly as entered")
        self.assertEqual(invoice.credit_outstanding, Decimal("200.0000"))

        counter, events = self._cash_counter_vs_events()
        self.assertEqual(counter, events)

    def test_advance_exactly_equal_to_total_is_untouched(self):
        """Boundary: the cap triggers on `>`, not `>=`."""
        product = self.make_stocked_product(stock=20, unit_cost="50", selling_price="100")
        invoice = self._advance_invoice(product, quantity=3, advance="300")
    ...
    def test_cash_reflects_only_the_capped_amount(self):
        """Asserts the VALUE, not just internal agreement — both could be
        wrong together if the event were also over-recorded."""
        from cash_flow.models import CashFlow

        before = CashFlow.get_instance().cash_in_hand
        product = self.make_stocked_product(stock=20, unit_cost="50", selling_price="100")
        after_stock = CashFlow.get_instance().cash_in_hand

        invoice = self._advance_invoice(product, quantity=2, advance="5000")
        after = CashFlow.get_instance().cash_in_hand

        self.assertEqual(
            after - after_stock, invoice.grand_total,
            "cash should have risen by the capped advance (200), not the 5,000 entered",
        )
