...
## Always O(1), always DRY

- **Default to O(1) reads.** Any figure shown on a dashboard, report header, or "all-time"/no-filter stat block should be a stored field read off a singleton (or otherwise pre-computed), not a live aggregate over a growing table — even if the live aggregate "would work fine today." If a new report or stat needs a number that isn't already stored anywhere, check whether the owning app's Flow singleton (or equivalent) can grow a field for it before reaching for `Sum()`/`Count()` over history. Live aggregation is only acceptable when it's genuinely bounded and can't grow with history — e.g. a per-month trend query (bounded by month count, not row count) or a live snapshot report (Inventory Valuation — bounded by today's product count). When in doubt, flag it to the user rather than silently shipping an O(n) read as if it were free (see the `ProfitFlow.total_gross_profit`/`total_net_gross_profit` gap caught and fixed this way).
- **Always follow DRY.** Before writing a new view/template/function, check whether an existing one can be generalized instead of copy-pasted per instance. This project's reports print feature is the model: one generic `BaseReportPrintView` + one generic PDF template serve all 12+ reports, instead of one bespoke view/template per report — because the reports are all the same shape (title + filter + stats + table). The one exception is genuinely uniquely-structured documents (an invoice, a purchase order) — those legitimately get their own template, because they aren't interchangeable with each other. When adding a new "trigger this calculation" mechanism, prefer calling the existing public selector/service that already does the loop (e.g. `get_asset_stats()`, `get_cash_management_stats()`) over reaching into private internals or re-implementing the loop.
...
- **No cron/celery.** Time-based calculations (depreciation, investor growth, monthly profit finalization, etc.) use a "catch-up on view" pattern instead — a function that posts any missing periods when the relevant object/stats endpoint is actually read. Triggered by real requests, not a scheduler.
- **Every catch-up-on-view mechanism must be registered in `GET /api/system/catch-up/`** (`backend/backend/views.py` `TriggerAllCatchUpsView`) — this endpoint exists specifically so freshness of these lazily-computed figures never depends on which pages a user happens to visit. It's called once on every dashboard load (`AdminDashboard.jsx`) and must survive any future dashboard rewrite. If you add a 4th catch-up mechanism anywhere in the codebase, wire it in here too, in dependency order relative to the existing three (assets → investors → profits).
...
- **Snapshot historical figures at the moment they're locked in**, never compute them live from current state later — e.g. `MonthlyProfitInvestorShare.share_percent_snapshot` freezes an investor's ownership % as of that month forever, even if their live % changes afterward. Same pattern as `name_snapshot`/`category_name_snapshot`/`rate_applied` elsewhere. If a figure represents "what was true back then," store it; don't derive it from today's data.
- **Never conflate a distribution of earnings with a movement of capital.** A profit payout to an investor (`InvestorProfitPayout`) must never touch `net_stake`/`total_invested`/`total_withdrawn` — those fields represent capital, not earnings. When a payout is reinvested, model it as two honest, separately-reversible ledger entries (cash out via the payout, cash back in via a real `InvestorTransaction`), not a single combined record — this keeps the audit trail truthful and each half independently reversible.
- **Any page showing a computed-but-not-certified financial figure** (business worth, monthly profit, investor payouts, tax positions) gets the standard amber disclaimer banner ("This is an internal estimate, not a certified valuation/filing... The developer is not responsible at all for decisions made from this page.") — copy the existing banner from `BusinessWorthPage.jsx`/`TaxesPage.jsx` rather than inventing new wording.
